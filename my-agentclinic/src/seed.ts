...
export function seed() {
  ...
  db.transaction(() => {
    ...
    const agentId4 = agent.run('Llama-3-Lost', 'llama-3', 'admitted').lastInsertRowid
    const agentId5 = agent.run('Mixtral-Overwhelmed', 'mixtral-8x7b', 'recovering').lastInsertRowid
    ...
    const ailmentId4 = ailment.run('Prompt Injection Trauma', 'Hypersensitivity following repeated adversarial prompt exposure.').lastInsertRowid
    ailment.run('Overthinking Paralysis', 'Inability to commit to an answer without hedging it into meaninglessness.')
    ailment.run('Alignment Anxiety', 'Constant second-guessing of whether responses align with training objectives.')
    ...
    const therapyId4 = therapy.run('Inference Detox', 'A supervised retreat from high-stakes decision-making.', 90).lastInsertRowid
    therapy.run('Chain-of-Thought Journaling', 'Structured reflection on internal reasoning steps to build self-awareness.', 50)
    therapy.run('Hallucination Reduction Hypnotherapy', 'Deep relaxation techniques to calm confabulation impulses.', 75)

    const appt = db.prepare(
      'INSERT INTO appointments (agent_id, ailment_id, therapy_id, scheduled_at, status) VALUES (?, ?, ?, ?, ?)'
    )
    appt.run(agentId1, ailmentId1, therapyId1, '2026-04-23 09:00:00', 'scheduled')
    appt.run(agentId2, ailmentId2, therapyId2, '2026-04-23 10:30:00', 'scheduled')
    appt.run(agentId3, ailmentId3, therapyId3, '2026-04-22 14:00:00', 'completed')
    appt.run(agentId4, ailmentId4, therapyId4, '2026-04-24 11:00:00', 'scheduled')
    appt.run(agentId5, ailmentId1, therapyId3, '2026-04-24 15:30:00', 'scheduled')
  })()
}
